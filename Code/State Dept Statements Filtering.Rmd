...
kim_texts <- kim_texts %>%
  mutate(mention = str_extract(text, "[A-Za-z'-]+ Kim+ [A-Za-z'-]+")) %>%
...
Remove_Kims <- function(i){
  return(str_detect(i, "Ambassador|Chloe|Sung|Yong-chol", negate = T)) #detecting other Kims besides KJU
}

state_kim_texts <- kim_texts %>%
...
save(state_kim_texts, file = "state_kim_texts.RData")
